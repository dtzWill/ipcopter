// XXX: WD: Spliced from pieces of the Zeus project:
// 	https://github.com/burke/zeus/tree/master/go/unixsocket
...
func NewFromConn(C net.Conn) (*Usock, error) {
	unixConn, ok := C.(*net.UnixConn)
	if !ok {
		return nil, errors.New(fmt.Sprintf("unexpected type; expected UnixConn, got %T", unixConn))
	}
